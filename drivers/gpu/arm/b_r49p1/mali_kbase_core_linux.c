...
#include "uapi/gpu/arm/b_r49p1/mali_kbase_mem_profile_debugfs_buf_size.h"
...
#include <uapi/gpu/arm/b_r49p1/mali_kbase_ioctl.h>
#if !MALI_USE_CSF
...
#include <linux/b_r49p1/mali_hw_access.h>
...
int kbase_device_coherency_init(struct kbase_device *kbdev)
{
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (!kbase_device_supports_coherency_mode(kbdev, override_coherency)) {
                /* r49p0 porting: This error should be only for GPU_ID_PRODUCT_TMIX GPU */
			//err = -EINVAL;
			goto early_exit;
		}
                /* For sapphire COHERENCY_NONE = 31(0x1F) should be set to avoid GPU Fault*/
                /* In dts file system-coherency is set to 0 but it should be 31 */
